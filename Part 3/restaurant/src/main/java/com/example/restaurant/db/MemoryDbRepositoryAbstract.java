...
public abstract class MemoryDbRepositoryAbstract<T extends MemoryDbEntity> implements MemoryDbRepositoryIfs<T> {
    ...
    public T save(T entity) {
        ...
        if(optionalEntity.isEmpty()){ // db에 데이터가 있는 경우
            index++;
    ...
    public void deleteById(int idx) {
        var optionalEntiy = db.stream().filter(it -> it.getIndex() == index).findFirst();
        if(optionalEntiy.isPresent()){
            db.remove(optionalEntiy.get());
        }
    }
